...
class RouterImpl implements Router {
  ...
  private async build_response(
    ...
    let headers: Record<string, string> = { ...set.headers }

    // Handle different content types
    if (set.content === 'json' || (set.content === 'auto' && typeof result === 'object' && result !== null)) {
      ...
      body = String(result)
    } else if (set.content === 'png' || set.content === 'svg' || set.content === 'gif' || set.content === 'webp') {
    ...
    // Set cache headers
    if (set.cache) {
      headers['Cache-Control'] = `public, max-age=${this.parse_cache_duration(set.cache)}`
    }
   ...
   * @param handler - Route handler function
   */
  get<Path extends string>(path: Path, middlewares: Middleware[], handler: RouteHandler<Path>): void
   ...
   * @param handler - Route handler function
   */
  post<Path extends string>(path: Path, middlewares: Middleware[], handler: RouteHandler<Path>): void
   ...
   * @param handler - Route handler function
   */
  put<Path extends string>(path: Path, middlewares: Middleware[], handler: RouteHandler<Path>): void
   ...
   * @param handler - Route handler function
   */
  patch<Path extends string>(path: Path, middlewares: Middleware[], handler: RouteHandler<Path>): void
   ...
   * @param handler - Route handler function
   */
  delete<Path extends string>(path: Path, middlewares: Middleware[], handler: RouteHandler<Path>): void
   ...
   * @param handler - Route handler function
   */
  options<Path extends string>(path: Path, middlewares: Middleware[], handler: RouteHandler<Path>): void
   ...
   * @param handler - Route handler function
   */
  head<Path extends string>(path: Path, middlewares: Middleware[], handler: RouteHandler<Path>): void
   ...
   * @param handler - Route handler function
   */
  all<Path extends string>(path: Path, middlewares: Middleware[], handler: RouteHandler<Path>): void
