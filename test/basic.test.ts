...
test('POST request with body', async () => {
  ...
  const request_body = JSON.stringify({ name: 'Alice', email: 'alice@example.com' })
  const response = await server.fetch(new Request('http://localhost/api/users', {
    method: 'POST',
...
test('response helpers', async () => {
  ...
  // Test text response
  const text_response = await server.fetch(new Request('http://localhost/text'))
  expect(text_response.status).toBe(200)
  expect(text_response.headers.get('content-type')).toBe('text/plain')
  
  // Test HTML response
  const html_response = await server.fetch(new Request('http://localhost/html'))
  expect(html_response.status).toBe(200)
  expect(html_response.headers.get('content-type')).toBe('text/html')
  
  // Test custom status
  const status_response = await server.fetch(new Request('http://localhost/status'))
...
test('middleware functionality', async () => {
  ...
  const route_middleware = async ({}, next) => {
  ...
  const response = await server.fetch(new Request('http://localhost/middleware'))
...
test('wildcard routes', async () => {
  ...
  // Test wildcard match
  const response1 = await server.fetch(new Request('http://localhost/api/posts'))
  ...
  const response2 = await server.fetch(new Request('http://localhost/api/posts/123'))
...
test('query parameters', async () => {
  ...
  const response = await server.fetch(new Request('http://localhost/search?q=test&page=2'))
