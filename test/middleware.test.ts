...
import { create_router, create_server, error_handler, HttpError, cors, logger, create_health_check } from '../src/index'
...
test('error handler middleware - HttpError', async () => {
  ...
  const response = await server.fetch(new Request('http://localhost/not-found'))
...
test('HttpError factory methods', () => {
  const bad_request = HttpError.bad_request('Invalid input', { field: 'email' })
...
test('combined middleware', async () => {
  ...
  // Test success case
  const response1 = await server.fetch(new Request('http://localhost/api/success'))
  expect(response1.status).toBe(200)
  expect(response1.headers.get('Access-Control-Allow-Origin')).toBe('*')

  // Test error case
  const response2 = await server.fetch(new Request('http://localhost/api/error'))
